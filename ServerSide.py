#Overview
#Client request to write file
#Server grants request and connection is opened
#File is sent in blocks of 512B
#After sending a block of 512B an acknowledgement must be received then another can be sent
#File smaller than 512B implies end of transfer
#If timeout occurs retransmit ack or data
#If error occurs on client or server side an error packet is sent to the peer
#Client or server closes connection after sending error packet
#If server or client receives error packet the connection is closed
#If error packet is lost the server or client will just timeout and close the connection
#Errors occur if file not found, unknown packet received (unknown format), software issue (no more space or no access)
#Only error with no termination is source port of client is incorrect
#Error packet sent to client no termination
#TCP Style Sliding window:
#Starting at block number 0 send x number of packets
#If ACK received for each packet send blocks x through x+x and continue
#If an ACK is not received move to the block number of lowest block number...
#Send x number of packets starting from that block number
...
def openFile(fileName, socket):
    file = open("uploaded/" + fileName, "a")
    return file

def createFile(fileName, socket):
    fname = "uploaded/" + fileName
    if os.path.isfile(fname):
        sendError(socket, "File already exists on server!", 6)
        #os.remove(fname)
    file = open(fname, "a")
    return file
...
def mainNoSW():
    ...
    while True:
        ...
        if opcode == 3:
            ...
            if "error" in status:
                ...
                print("This is the final block of data.")
    ...
    throughput = os.path.getsize('uploaded/' + FILE_NAME)/elapsedTime
